...
def extract_major_sections(content: str) -> Dict[str, str]:
    ...
    for i in range(len(headings)):
        ...
        start_idx = next(
            (idx for idx, text in enumerate(elements_list) if heading in text), -1
        )
                ...
                end_idx = next(
                    (
                        ...
                        for idx, text in enumerate(
                            elements_list[start_idx + 1 :], start_idx + 1
                        )
...
def filter_relevant_sections(
    sections: Dict[str, str], client: OpenAI
) -> Dict[str, str]:
    ...
    section_previews = {
        title: content[:200] + "..." for title, content in sections.items()
    }
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert at identifying relevant sections in SEC filings.",
                },
                {"role": "user", "content": prompt},
                {
                    "role": "assistant",
                    "content": "I will identify the most relevant sections and return them as a JSON array.",
                },
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert at extracting executive compensation and biographical information from SEC filings.",
                },
                {"role": "user", "content": prompt},
                {
                    "role": "assistant",
                    "content": "I will extract the executive information and return it in the requested JSON format.",
                },
                {
                    "role": "user",
...
def identify_headings(content: str) -> List[Tuple[str, float]]:
    ...
    for elem in soup.find_all(
        style=re.compile("(font-weight.*?bold|font-size.*?1[2-9]px)", re.I)
    ):
...
def extract_sections(content: str, headings: List[Tuple[str, float]]) -> Dict[str, str]:
    ...
    # Get all text elements
    text_elements = [
        elem.strip() for elem in soup.find_all(string=True) if elem.strip()
    ]
    ...
    for i, (heading, _) in enumerate(headings):
        try:
            start_idx = next(
                idx for idx, text in enumerate(text_elements) if heading in text
            )
            ...
            for next_heading, _ in headings[i + 1 :]:
                try:
                    next_idx = next(
                        idx
                        for idx, text in enumerate(
                            text_elements[start_idx + 1 :], start_idx + 1
                        )
                        if next_heading in text
                    )
                    if next_idx < end_idx:
...
def get_compensation_section(sections: Dict[str, str]) -> Optional[Dict[str, str]]:
    ...
    for heading, content in sections.items():
        ...
        for category, keywords in section_keywords.items():
            if any(k in heading_lower for k in keywords) or any(
                k in content.lower()[:1000] for k in keywords
            ):
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        for cik, name, filing_date, file_path in companies:
            try:
                ...
                status = conn.execute(
                    "SELECT status FROM processing_status WHERE cik = ?", (cik,)
                ).fetchone()
