...
class EDGARValidator:
    ...
    def get_filing_links(
        self, cik: str, filing_type: str = "DEF 14A", limit: int = 1
    ) -> list[dict]:
    ...
    def get_filing_content(self, doc_url: str) -> str | None:
        ...
        try:
            ...
            if not filing_link:
                ...
                for link in soup.find_all("a"):
                    ...
                    if ".htm" in href and "def 14a" in text:
                        filing_link = f"https://www.sec.gov{href}"
                        self.logger.info(
                            f"Found DEF 14A link (alternative method): {filing_link}"
                        )
                        break
            ...
            if "proxy statement" in content.lower():
                ...
                self.logger.warning(
                    "Retrieved content does not appear to be a proxy statement"
                )
                return None
    ...
    def process_company(self, cik: str, output_dir: str = "def14a_filings"):
        ...
        for filing in filings:
            ...
            if self.validate_filing_content(content):
                ...
                self.logger.warning(
                    f"Invalid or empty filing found for CIK {cik} dated {filing['filing_date']}"
                )
